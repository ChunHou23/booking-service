...
func (m *postgresDBRepo) UpdateProcessedForReservation(processed, id int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m *postgresDBRepo) DeleteBlockForRoomByID(restrictionID int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
